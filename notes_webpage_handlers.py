...
def login():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "select * from Users where username='{}'".format(login_data['user_name']))
...
def registration():
    if request.method == 'POST':
        ...
        cursor.execute(
            "select * from Users where username='{}'".format(registration_data['user_name']))
        query_result = cursor.fetchone()
...
def note_delete():
    if g.user:
        ...
        cursor.execute(
            'update Notes set deleted="{}" where id ="{}"'.format(1, notes_data['Id']))
        conn.commit()
...
def trash_restore():
    if g.user:
        ...
        cursor.execute(
            'update Notes set deleted="{}" where id="{}"'.format(0, notes_data['Id']))
        conn.commit()
...
def trash_delete():
    if g.user:
        ...
        cursor.execute(
            'delete from Notes where id="{}"'.format(notes_data['Id']))
        conn.commit()
